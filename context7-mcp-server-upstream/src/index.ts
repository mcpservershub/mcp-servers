...
// Helper function to write output to file with error handling
async function writeOutputToFile(outputFile: string | undefined, content: string): Promise<void> {
  ...
  try {
    ...
    console.error(`Failed to write output to file: ${outputFile}`, error);
    throw new Error(`Failed to write output to file: ${error instanceof Error ? error.message : "Unknown error"}`);
  }
}
...
For ambiguous queries, request clarification before proceeding with a best-guess match.`,
    {
      ...
      output_file: z
        .string()
        .optional()
        .describe("Optional file path to write the output to. If provided, the tool output will be saved to this file."),
    },
    async ({ libraryName, output_file }) => {
      ...
      const outputText = `Available Libraries (top matches):
...
${resultsText}`;
      ...
      if (output_file) {
        try {
          await writeOutputToFile(output_file, outputText);
        } catch (error) {
          return {
            content: [
              {
  ...
  server.tool(
    ...
    "Fetches up-to-date documentation for a library. You must call 'resolve-library-id' first to obtain the exact Context7-compatible library ID required to use this tool, UNLESS the user explicitly provides a library ID in the format '/org/project' or '/org/project/version' in their query.",
    {
      ...
      output_file: z
        .string()
        .optional()
        .describe("Optional file path to write the documentation to. If provided, the documentation will be saved to this file."),
    },
    async ({ context7CompatibleLibraryID, tokens = DEFAULT_MINIMUM_TOKENS, topic = "", output_file }) => {
      ...
      if (!fetchDocsResponse) {
        const errorMessage = "Documentation not found or not finalized for this library. This might have happened because you used an invalid Context7-compatible library ID. To get a valid Context7-compatible library ID, use the 'resolve-library-id' with the package name you wish to retrieve documentation for.";
        return {
          content: [
            {
              type: "text",
      ...
      if (output_file) {
        try {
          await writeOutputToFile(output_file, fetchDocsResponse);
        } catch (error) {
          return {
            content: [
              {
