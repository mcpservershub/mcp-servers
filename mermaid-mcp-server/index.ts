...
async function renderMermaidPng(
  ...
  try {
    ...
    log(
      LogLevel.ERROR,
      `Error in renderMermaidPng: ${error instanceof Error ? error.message : String(error)}`,
    );
    log(
      LogLevel.ERROR,
...
runServer().catch((error) => {
  log(
    LogLevel.CRITICAL,
    `Fatal error running server: ${error instanceof Error ? error.message : String(error)}`,
  );
  process.exit(1);
});
