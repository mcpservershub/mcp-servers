...
While using the MCP Server with Docker containers, the `FIRECRAWL_API_KEY` need to be provided as `--build-arg` or can be passed with an
`.env` file using `--env-file` flag during building the Docker image 
