        ...
        async def handle_call_tool(
            ...
            try:
                if name == "get-connection-settings":
                    ...
                    if not is_healthy:
                        # Provide more context when health check fails
                        return [
                            types.TextContent(
                                type="text",
                                text=f"Meilisearch is unavailable at {self.url}. Please ensure the server is running and accessible.",
                            )
                        ]
                    return [
                        types.TextContent(
                            type="text",
                            text="Meilisearch is healthy and available",
                        )
                    ]
                ...
                # Re-raise the exception to let MCP handle it properly
                # This will send an error response instead of a success with error text
                raise
