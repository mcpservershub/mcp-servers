...
`,
  inputSchema: {
    ...
    properties: {
      ...
      output_file: {
        type: 'string',
        description: 'Optional file path to save the scraped content',
      },
    },
    required: ['url'],
  },
...
`,
  inputSchema: {
    ...
    properties: {
      ...
      output_file: {
        type: 'string',
        description: 'Optional file path to save the discovered URLs',
      },
    },
    required: ['url'],
  },
...
`,
  inputSchema: {
    ...
    properties: {
      ...
      output_file: {
        type: 'string',
        description: 'Optional file path to save the crawl results when completed',
      },
    },
    required: ['url'],
  },
...
`,
  inputSchema: {
    ...
    properties: {
      ...
      location: {
        type: 'string',
        description: 'Location for search results (e.g., "United States", "UK", etc.)',
      },
      scrapeOptions: {
        type: 'object',
        properties: {
...
`,
  inputSchema: {
    ...
    properties: {
      ...
      output_file: {
        type: 'string',
        description: 'Optional file path to save the extracted data',
      },
    },
    required: ['urls'],
  },
...
`,
  inputSchema: {
    ...
    properties: {
      ...
      output_file: {
        type: 'string',
        description: 'Optional file path to save the research results',
      },
    },
    required: ['query'],
  },
...
`,
  inputSchema: {
    ...
    properties: {
      ...
      output_file: {
        type: 'string',
        description: 'Optional file path to save the generated LLMs.txt file',
      },
    },
    required: ['url'],
  },
...
const FIRECRAWL_MODE = process.env.FIRECRAWL_MODE;

// Helper function to detect if running in local mode
function isLocalMode(): boolean {
  // Check if explicitly set to local mode
  if (FIRECRAWL_MODE === 'local') return true;
  
  // Auto-detect local instance by URL
  if (FIRECRAWL_API_URL) {
    const url = FIRECRAWL_API_URL.toLowerCase();
...
if (!IS_LOCAL_MODE && process.env.CLOUD_SERVICE !== 'true' && !FIRECRAWL_API_KEY) {
  console.error(
    'Error: FIRECRAWL_API_KEY environment variable is required when using the cloud service'
  );
  console.error(
    'For local development, set FIRECRAWL_API_URL to your local instance (e.g., http://localhost:3002)'
  );
  console.error(
    'Or set FIRECRAWL_MODE=local to skip API key validation'
  );
  process.exit(1);
}
...
if (IS_LOCAL_MODE) {
  console.error(`Running in LOCAL mode with API URL: ${FIRECRAWL_API_URL || 'http://localhost:3002'}`);
} else {
  console.error('Running in CLOUD mode');
}
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    ...
    if (IS_LOCAL_MODE) {
      // For local mode, use a dummy key or no key
      apiKey = 'local-dev-key';
    } else if (process.env.CLOUD_SERVICE) {
      apiKey = request.params._meta?.apiKey as string;
      if (!apiKey) {
    ...
    const client = new FirecrawlApp({
      apiKey,
      apiUrl: FIRECRAWL_API_URL || (IS_LOCAL_MODE ? 'http://localhost:3002' : undefined),
    });
    ...
    switch (name) {
      case 'firecrawl_scrape': {
        ...
        try {
          ...
          // Add warning to response if present (skip in local mode)
          if (!IS_LOCAL_MODE && response.warning) {
          ...
          if ((args as any).output_file) {
            await saveToFile((args as any).output_file, finalContent);
          }
          ...
          return {
            content: [
              {
                type: 'text',
                text: trimResponseText(finalContent),
              },
            ],
            isError: false,
          };
      ...
      case 'firecrawl_map': {
        ...
        const linksContent = response.links.join('\n');
        
        // Save to file if output_file is specified
        if ((args as any).output_file) {
          await saveToFile((args as any).output_file, linksContent);
        }
        
        return {
          content: [
            { type: 'text', text: trimResponseText(linksContent) },
          ],
          isError: false,
        };
      }
...
${result.markdown ? `\nContent:\n${result.markdown}` : ''}`
          ...
          // Save to file if output_file is specified
          if ((args as any).output_file) {
            await saveToFile((args as any).output_file, results);
          }
      ...
      case 'firecrawl_extract': {
        ...
        try {
          ...
          const extractedData = JSON.stringify(response.data, null, 2);
          
          // Save to file if output_file is specified
          if ((args as any).output_file) {
            await saveToFile((args as any).output_file, extractedData);
          }
          ...
          const result = {
            content: [
              {
                type: 'text',
                text: trimResponseText(extractedData),
              },
            ],
            isError: false,
          };
          ...
          // Special handling for local/self-hosted instance errors
          if (
            (IS_LOCAL_MODE || FIRECRAWL_API_URL) &&
            errorMessage.toLowerCase().includes('not supported')
          ) {
            safeLog(
              'error',
              'Extraction requires LLM support which may not be configured in your local instance'
            );
            return {
              content: [
                {
                  type: 'text',
                  text: trimResponseText(
                    'Extraction requires LLM support. Please ensure your local Firecrawl instance has LLM (OpenAI/Ollama) configured.'
                  ),
                },
              ],
              isError: true,
      ...
      case 'firecrawl_deep_research': {
        ...
        try {
          ...
          // Save to file if output_file is specified
          if ((args as any).output_file) {
            await saveToFile((args as any).output_file, JSON.stringify(formattedResponse, null, 2));
          }
      ...
      case 'firecrawl_generate_llmstxt': {
        ...
        try {
          ...
          // Save to file if output_file is specified
          if ((args as any).output_file) {
            await saveToFile((args as any).output_file, resultText);
          }
...
async function saveToFile(filePath: string, content: string): Promise<void> {
  try {
    ...
    safeLog('info', `Content saved to file: ${filePath}`);
  } catch (error) {
    safeLog('error', `Failed to save to file ${filePath}: ${error}`);
...
async function runLocalServer() {
  try {
    ...
    safeLog(
      'info',
      `Configuration: Mode: ${IS_LOCAL_MODE ? 'LOCAL' : 'CLOUD'}, API URL: ${FIRECRAWL_API_URL || (IS_LOCAL_MODE ? 'http://localhost:3002' : 'https://api.firecrawl.dev')}`
    );
